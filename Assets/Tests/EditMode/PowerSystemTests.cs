...
namespace LastHope.Tests.EditMode
{
    public class PowerSystemTests
    {
        ...
        public void Allocate_CriticalPoweredBeforeNormal_WhenSupplyInsufficient()
        {
            ...
            shelter.PlacedModules["slot_utility_area_1"] = new BuiltModuleState
                { ModuleId = ShelterModuleIds.Pump, Priority = PowerPriority.Normal };
            shelter.PlacedModules["slot_water_processing_1"] = new BuiltModuleState
            ...
            Assert.IsFalse(shelter.PlacedModules["slot_utility_area_1"].Powered, "Không đủ điện, Normal bị cắt trước.");
            Assert.IsFalse(shelter.PlacedModules["slot_water_processing_1"].Powered, "Grid=0 và không có Battery.");
        }
        ...
        public void Allocate_CriticalGetsPower_NormalCutWhenSupplyRunsOut()
        {
            ...
            Assert.IsTrue(shelter.PlacedModules["slot_water_processing_1"].Powered, "Critical được ưu tiên dùng Battery.");
            Assert.IsFalse(shelter.PlacedModules["slot_utility_area_1"].Powered, "Battery hết sau khi trả Critical.");
        }
        ...
        public void Allocate_Disabled_NeverPowered()
        {
            ...
            Assert.IsFalse(shelter.PlacedModules["slot_utility_area_1"].Powered);
        }
        ...
        {
            var shelter = new ShelterState { BatteryCharge = 0f };
            shelter.PlacedModules["slot_upper_living_1"] = new BuiltModuleState
                { ModuleId = ShelterModuleIds.ElevatedStorage, Priority = PowerPriority.Normal };

            PowerSystem.Allocate(shelter, definitions, DisasterPhase.RouteClosure); // supply = 0

            Assert.IsTrue(shelter.PlacedModules["slot_upper_living_1"].Powered, "power_demand=0 không cần cấp điện.");
        }
    }
}

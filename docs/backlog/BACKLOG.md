...
| ID | Hạng mục | Trạng thái | Ghi chú |
| --- | --- | --- | --- |
| BL-P3-01 | Main Shelter blockout | Verify | 8 Zone (`shelterzones_p3.json`) hiển thị trong `ShelterPanel` (không dựng phòng riêng từng Zone). **Có Ground/Upper Floor vật lý thật kiểu Z-level Project Zomboid** — dựng lại 5 lần sau nhiều vòng user review (2026-07-28→29): (1) SetActive nhị phân + bấm E, (2) 2 vùng trigger đổi tức thời không bấm phím, (3) 1 `StaircaseZone` leo dần dùng `Collider2D`/`OnTrigger*2D`, (4) bỏ Collider2D — so khoảng cách thuần trong `Update()`, (5) **bỏ luôn khái niệm "hướng đang leo" (suy 1 lần từ CurrentFloor cũ rồi cache) — `BlendT` tính thuần theo vị trí Y mỗi frame, không phụ thuộc lịch sử** — user báo 2 bug ở bản (4): tầng dưới rõ hẳn (không còn mờ) sau khi leo xong, và 2 đầu cầu thang xử lý không đối xứng (đầu 2 vào đã mờ nhất ngay). Nguyên nhân gốc: hướng leo suy từ `CurrentFloor` có thể lệch nếu đứng nán ở biên vùng. `PlayerFloorState` đổi model: `TransitioningToFloor`/`ClimbProgress` → `BlendLowerFloor`/`BlendUpperFloor`/`BlendT`/`IsBlending`, `UpdateBlend`/`EndBlend` thay `BeginClimb`/`SetClimbProgress`/`CancelClimb`. Chi tiết: `docs/plans/2026-07-29-staircase-blend-fix.md`. Roof vẫn chỉ là logic (không module nào target zone `roof`). Fix kèm: bug `FindObjectsByType` loại trừ GameObject inactive khiến tầng chưa từng active không bao giờ bật lại được (sửa dùng `FindObjectsInactive.Include`); sprite cầu thang phủ đúng kích thước vùng + màu hổ phách riêng biệt |
...
| BL-P3-03 | Build và Placement | Verify | Viết lại theo Free Placement (2026-07-28, `docs/plans/2026-07-28-free-placement.md`): `ShelterZoneDefinition` có world bounds thay Slot ID cố định, `BuildSystem.CanPlaceAt` validate world position + overlap (`ModuleDefinition.FootprintRadius`), `PlacementModeController` (Presentation) hiện ghost theo chuột + khung mờ biên Zone, click xác nhận. User playtest phát hiện 3 bug UX liên tiếp (2026-07-29), đã sửa: (1) ghost đổi màu đỏ lệch hẳn so với chuột — do scene `00_Boot` không unload nên có 2 Camera cùng tag `MainCamera` song song, `Camera.main` (chỗ duy nhất trong code dùng API này) có lúc trả nhầm BootCamera đứng yên; sửa: `BootLoader` tự unload scene Boot sau khi Persistent sẵn sàng + `PlacementModeController` dùng field `worldCamera` tường minh (wire qua `SceneSetup.cs`) thay `Camera.main`; (2) không có chữ hướng dẫn/lý do reject trên màn hình khi đang Placement Mode — thêm khung OnGUI hiện tên Module + lý do đỏ (dịch tiếng Việt) + "ESC: Huỷ"; (3) thiếu vật liệu chỉ báo sau khi đã vào Placement Mode dù không phụ thuộc vị trí — tách `BuildSystem.HasEnoughMaterials` dùng chung, `ShelterPanel` disable nút "Chọn vị trí" + hiện "— thiếu vật liệu" ngay tại danh sách Zone, chặn từ trước khi vào Placement Mode. Sau đó user báo thêm: xây xong Module không thấy hiện trong thế giới (chỉ ghost lúc đặt, xây xong không có visual nào) — thêm `PlacedModuleRenderer` (Presentation, wire trong `SceneSetup.cs` scene Shelter), tự đồng bộ toàn bộ `ShelterState.PlacedModules` mỗi khi nghe `ConstructionCompleted`/`ModuleDismantled` (event mới) — tương tự nguyên tắc "tính lại từ state hiện tại, không cache" đã rút ra từ bug cầu thang: mỗi Module xây xong hiện sprite tại đúng `(PositionX,PositionY)`, parent đúng `FloorLevel` (Ground/Upper) để tự mờ/ẩn theo Z-level. Tiếp theo user yêu cầu đổi hẳn cách Tháo: thêm `PlacedModuleHoverMenu` (Presentation, world-space) — rê chuột dừng trên sprite Module (trong tầm + đúng tầng player) hiện menu nhỏ nút "Tháo", menu neo theo **vị trí world của Module** (không neo theo chuột — bug ban đầu khiến menu "chạy theo" chuột không bấm được) + logic sticky giữ menu mở khi chuột rời world-hover nhưng còn trong chính cái menu. Bỏ hẳn nút "Tháo" khỏi `ShelterPanel` (chỉ còn nút đổi Power Priority). Tháo giờ không xoá mất Module — sinh 1 item "đã gói" (`ModuleDefinition.PackedItemId`, 5 item mới `item_packed_*` trong `items_p3_packed_modules.json`) vào Storage, đặt lại **tức thì không tốn Materials/BuildMinutes** qua nút "Đặt lại (×N)" mới trong `ShelterPanel` → `BuildSystem.RedeployModule`/`CanRedeployAt` (tách hình học dùng chung `CanPlaceGeometry` với `CanPlaceAt`, không qua `ConstructionState`) — khác hẳn xây mới, coi như bê nguyên Module đã tháo rời sang chỗ khác. Simplification có chủ đích: Tháo/Đặt lại reset Durability về 100%, không giữ hao mòn cũ (`ItemInstanceState.Condition` là per-slot, giữ đúng theo từng Module cần model dữ liệu phức tạp hơn nhiều). **Outdoor placement (Location ngoài trời) chưa làm** — không có Module Outdoor/Hybrid nào trong content để đặt (`modules_p3.json` cả 5 Module đều target Shelter), cần quyết định nội dung trước |
...
**Gate P3:** chưa chạy — toàn bộ BL-P3-01..18 đã có code (225 EditMode test), chờ user

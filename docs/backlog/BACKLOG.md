...
| ID | Hạng mục | Trạng thái | Ghi chú |
| --- | --- | --- | --- |
| BL-P3-01 | Main Shelter blockout | Verify | 8 Zone (`shelterzones_p3.json`) hiển thị trong `ShelterPanel` (không dựng phòng riêng từng Zone). **Có Ground/Upper Floor vật lý thật kiểu Z-level Project Zomboid** — dựng lại 3 lần sau nhiều vòng user review (2026-07-28→29): (1) SetActive nhị phân + bấm E, (2) 2 vùng trigger đổi tức thời không bấm phím, (3) **1 vùng `StaircaseZone` duy nhất leo dần theo `ClimbProgress`** (0→1 theo vị trí Y thật, đi lùi thì tụt lại, chỉ đổi tầng thật ở đỉnh) — user phản hồi "không có cảm giác đang đi lên từng bậc" ở bản (2), giờ mới đúng cảm giác leo cầu thang thay vì cổng dịch chuyển. `FloorRenderController` nội suy alpha liên tục giữa 2 tầng lúc đang leo (không nhị phân). Roof vẫn chỉ là logic (không module nào target zone `roof`). Fix kèm: bug `FindObjectsByType` loại trừ GameObject inactive khiến tầng chưa từng active không bao giờ bật lại được (sửa dùng `FindObjectsInactive.Include`); sprite cầu thang đổi từ icon prop nhỏ sang phủ đúng kích thước vùng trigger + màu hổ phách riêng biệt |

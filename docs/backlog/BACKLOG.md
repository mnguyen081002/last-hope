...
| BL-P3-03 | Build và Placement | Done | Viết lại theo Free Placement (2026-07-28, `docs/plans/2026-07-28-free-placement.md`), sau đó **Production tách khỏi Placement + Ready to Claim** (2026-07-30, `docs/plans/2026-07-30-module-production-placement-loop.md` + `docs/plans/2026-07-30-production-claim-implementation.md`, thay thế 1 phần mô tả cũ bên dưới): Production tại Shelter Console không còn chọn Zone/vị trí (`BuildSystem.StartConstruction(moduleId)`), xong chuyển `ShelterState.ReadyToClaim` — bấm "Nhận" (`ClaimProductionCommand`) mới cộng packed item vào **túi Player**. Tháo Module cũng trả packed item về túi Player (trước đó vào Storage). Đặt từ túi qua nút "Đặt" trong `InventoryPanel` — Zone tự resolve theo tầng đang đứng + vị trí chuột (không còn chọn Zone trước), `PlacementModeController` không auto-teleport tầng nữa. Rotate (phím R, 90°/lần) + footprint hộp (`FootprintWidth`/`FootprintHeight`, thay hình tròn, có khung diện tích riêng theo dõi mouse) + `ModuleSpriteCatalog` (sprite theo hướng, `Assets/Resources/Art/ShelterModulesP3`) đã có sẵn trước đợt 2026-07-30 này. **Snap Grid (cell size 1.0, overlay lưới mờ) chưa làm** — chưa liên quan bug nào đã báo, để dành làm sau. User báo 2 bug lúc playtest sprite thật (2026-07-30): (1) Production vẫn bắt chọn vị trí — do code chưa migrate theo thiết kế mới dù đã viết design doc, đã sửa; (2) nhân vật đứng im (WASD vô tác dụng) sau khi Production xong — không xác định được nguyên nhân qua đọc code tĩnh, fix khả nghi duy nhất áp dụng (`PlacedModuleHoverMenu.MarkHover` thiếu check `rect.Contains`) — **user playtest lại 2026-07-31: không còn tái hiện**, coi như đã hết. Lịch sử trước 2026-07-30 (Zone/vị trí chọn trước lúc bắt đầu xây, giữ lại để biết bối cảnh các fix UX): ghost đổi màu đỏ lệch chuột do 2 Camera `MainCamera` song song (`BootLoader` tự unload scene Boot + `worldCamera` tường minh thay `Camera.main`); thiếu chữ hướng dẫn/lý do reject trên màn hình (khung OnGUI); thiếu vật liệu chỉ báo trễ (`BuildSystem.HasEnoughMaterials` tách riêng); xây xong không hiện trong thế giới (`PlacedModuleRenderer` mới); đổi UX Tháo sang world-space hover-menu (`PlacedModuleHoverMenu`, thay nút trong `ShelterPanel`) — Tháo sinh packed item (`ModuleDefinition.PackedItemId`, `items_p3_packed_modules.json`) đặt lại tức thì không tốn Materials/BuildMinutes qua `BuildSystem.RedeployModule`/`CanRedeployAt`. Simplification giữ nguyên: Tháo/Đặt lại reset Durability về 100%, không giữ hao mòn cũ. **Outdoor placement (Location ngoài trời) chưa làm** — không có Module Outdoor/Hybrid nào trong content, cần quyết định nội dung trước, không chặn Gate P3 |

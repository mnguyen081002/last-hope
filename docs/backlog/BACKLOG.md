...
| ID | Hạng mục | Trạng thái | Ghi chú |
| --- | --- | --- | --- |
| BL-P3-01 | Main Shelter blockout | Verify | 8 Zone (`shelterzones_p3.json`) hiển thị trong `ShelterPanel` (không dựng phòng riêng từng Zone). **Có Ground/Upper Floor vật lý thật kiểu Z-level Project Zomboid** — dựng lại 4 lần sau nhiều vòng user review (2026-07-28→29): (1) SetActive nhị phân + bấm E, (2) 2 vùng trigger đổi tức thời không bấm phím, (3) 1 `StaircaseZone` leo dần dùng `Collider2D`/`OnTrigger*2D`, (4) **cùng cơ chế leo dần nhưng bỏ hẳn Collider2D — chuyển sang so khoảng cách thuần trong `Update()`** sau khi user báo "không đi lên cầu thang được" ở bản (3) (nghi ngờ `OnTriggerStay2D` không bắn đều đặn, không chẩn đoán được chính xác qua kiểm tra tĩnh nên đổi sang cách chắc chắn hơn — không phụ thuộc lịch physics). `FloorRenderController` nội suy alpha liên tục giữa 2 tầng lúc đang leo (không nhị phân). Roof vẫn chỉ là logic (không module nào target zone `roof`). Fix kèm: bug `FindObjectsByType` loại trừ GameObject inactive khiến tầng chưa từng active không bao giờ bật lại được (sửa dùng `FindObjectsInactive.Include`); sprite cầu thang phủ đúng kích thước vùng + màu hổ phách riêng biệt |

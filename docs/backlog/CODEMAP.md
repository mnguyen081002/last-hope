...
| Build | `Systems/Shelter/BuildSystem.cs` | `CanPlaceAt`/`CanRedeployAt`, `StartConstruction/CancelConstruction/SetPaused/RedeployModule/DismantleModule`, `ApplyShortTick` | ✅ | Free Placement (2026-07-28, thay Slot cố định) — `CanPlaceAt(zoneId, x, y, moduleId)` validate world position trong bounds Zone + không chồng lấn `PlacedModules` khác (tổng `FootprintRadius`) + đủ vật liệu. Chỉ 1 construction cùng lúc (MVP). Tick mỗi phút qua `ShortTick` — tự chạy dù rời Shelter/Sleep (Passive Task = hệ quả của kiến trúc tick sẵn có, không cần abstraction riêng). `ShelterState.PlacedModules` key theo placementId tự sinh (`NextPlacementId` counter), không phải slot id. Đổi 2026-07-29 (user yêu cầu Tháo → đặt lại tức thì, xem dòng Module hover-menu ở Presentation): tách hình học dùng chung `CanPlaceGeometry` (Zone/Bounds/Overlap, không gồm `ConstructionInProgress`) cho cả `CanPlaceAt` (xây mới, + `ConstructionInProgress` + `HasEnoughMaterials`) và `CanRedeployAt` (đặt lại Module đã gói, + đủ ≥1 `ModuleDefinition.PackedItemId` trong Storage — reason riêng `NotEnoughPackedModules`). `RedeployModule` tạo `BuiltModuleState` ngay lập tức, không qua `ConstructionState`/`BuildMinutes`. `DismantleModule` giờ nhận thêm `definitions`, không xoá suông nữa mà cộng 1 `PackedItemId` vào Storage (trước đó không hoàn gì) |
...
| Shelter | `UI/Panels/ShelterPanel.cs` | — | ⬜ | P3, tự mở khi nghe `ShelterConsoleOpened` (toggle). Toàn bộ Zone trong một panel (phạm vi rút gọn — không đi bộ tới từng Zone vật lý). Overview (Structural/Water Intrusion/Clean-Untreated Water/Battery/Filter), banner 3 Event + nút xử lý, Construction hiện thị 1 chỗ (chỉ 1 cái chạy cùng lúc). Free Placement (2026-07-28) — mỗi Zone liệt kê Module xây được kèm nút "Chọn vị trí" (publish `BeginPlacementMode(..., redeploy: false)`, đóng panel, xem `PlacementModeController`) thay vì "Xây" trực tiếp; danh sách `PlacedModules` hiện theo `(x,y)` thật, nút đổi Power Priority xoay vòng. Sửa 2026-07-29 — nút "Chọn vị trí" giờ disable + hiện "— thiếu vật liệu" ngay tại danh sách Zone nếu không đủ nguyên liệu (`BuildSystem.HasEnoughMaterials`, tách từ `CanPlaceAt`), thay vì để user vào Placement Mode rồi mới biết (vật liệu không phụ thuộc vị trí đặt). Bỏ hẳn nút "Tháo" (chuyển sang world hover-menu, xem `PlacedModuleHoverMenu`). Thêm nút "Đặt lại (×N)" khi Storage có ≥1 item đã gói của Module đó (`InventoryOps.CountOf(storage, module.PackedItemId)`) — publish `BeginPlacementMode(..., redeploy: true)`, không gate theo `constructionBusy` (Redeploy không đụng ConstructionState) |

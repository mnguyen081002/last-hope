...
| `Assets/Game/Core/State/PlayerConditionState.cs` (S7) | `PlayerConditionState` + `StatusEffectState` + `IncapacitationState` enum | `Health/Stamina/Fatigue/Hunger/Thirst` 0-100 (Hunger/Thirst: 0=không cần, 100=cạn — ngược chiều Health), `BodyTemperatureC`, `StatusEffects:Dict<id,{Severity,AppliedAtMinute}>`, `Exposures:Dict<hazardId,float>` (cộng dồn, không clamp 0-100), `Incapacitation` | ✅ (ConditionOpsTests/ConditionSystemTests) |
...
| `Assets/Game/Core/Events/GameEvents.cs` | `IGameEvent` + 17 struct: `WorldTimeChanged`, `DisasterPhaseChanged` (S7: nay `DisasterPhaseSystem` thật sự publish), `RouteStateChanged`, `ShelterWarningRaised`, `TaskCompleted`, `EventDiscovered`, `InventoryChanged`, `NpcStateChanged`, `OverloadStateChanged` (S5), `WorldStateReloaded` (S5), `ItemTransferred` (S5), `SearchPointOpened`, `ContainerViewRequested` (S6), `TravelStarted`, `TravelCompleted` (S6), **`ConditionChanged`, `StatusEffectChanged`** (S7) | ⬜ |
...
| `Assets/Game/Data/Definitions/ItemDefinition.cs` | `ItemDefinition` | `Category`, `BaseWeightKg`, `BaseVolumeLiters`, `MaxStackSize`, `MaxDurability`, `WaterResistance`, `Tags`, `TwoHandCarry` (S5), **`UseEffects:Dict<string,float>`** (S7, key "thirst"/"hunger"/"health"/"stamina"/"fatigue" — `UseItemCommand` áp khi dùng), **`EquipSlot`**/**`Protection:Dict<string,float>`** (S7, field đặt trước cho S8 EquipItemCommand — chưa có content nào dùng) | 🟡 (qua ContentValidationTests) |
...
| `Assets/Game/Data/BalanceConfig.cs` (S5; +`ConditionBalance` S7) | `BalanceConfig`, `InventoryBalance`, `TravelBalance`, `NewGameBalance`, `ConditionBalance` | Object config duy nhất (không phải Definition list) — mọi hằng số tuning: capacity 15kg/25L, overload threshold 100%/130%, hard cap 150%, speed modifier, travel load factor, start location, **condition** (thirst/hunger per hour, fatigue per long-tick/travel, stamina regen, body temp drift/regen, wet gain/dry rate, cold/exposure/sick threshold, starvation floor, collapsed threshold) | ✅ (BalanceLoadTests) |
| `Assets/Game/Data/DefinitionRegistry.cs` | `DefinitionRegistry` | `DefinitionVersion`, `Balance` (S5), `Items/Locations/Routes/SearchPoints/**DisasterPhases**` (S7, IReadOnlyDictionary), `TryGetItem/Location/Route/SearchPoint/**DisasterPhase**`. Ctor: `disasterPhases` param optional (default empty dict) — không phá call site test cũ | ✅ (qua DefinitionLoaderTests) |
...
| `Assets/Game/Systems/Condition/ConditionSystem.cs` (S7) | `ConditionSystem` (plain C#) | ShortTick: stamina regen (×0.5 khi status `black_water_exposure`/`sick`), wet gain khi mưa ngoài trời/khô tại shelter (`LocationDefinition.IsShelter`), body temp drift (xuống khi wet>50 lúc mưa, lên tại shelter), cold status hysteresis (<35 bật, >36 tắt). LongTick: hunger/thirst/fatigue accrual, `ConditionOps.ApplyExposureStatusChain`, health decay khi đói/khát cạn (floor 1, không chết) + khi sick (không floor — chưa reachable tới khi S8 có nguồn exposure thật). Nghe `TravelCompleted` → +fatigue cố định | ✅ (ConditionSystemTests, 9 test) |
...
| File | Class | API chính | Test |
| --- | --- | --- | --- |
| `Assets/Game/DebugTools/Panel/DebugPanel.cs` | `DebugPanel` (MonoBehaviour, OnGUI, F2) | Xem World Time, Fast-forward clock, Pause/TimeScale (qua `SimulationDriver`), Add Item (bypass Command Layer — cheat có ghi rõ), **Condition cheat** (S7: xem Health/Stamina/Fatigue/Hunger/Thirst/BodyTemp/StatusEffects, ô nhập `stat name + delta` áp trực tiếp qua `ConditionOps`, bypass Command Layer), Save/Autosave + nút Load theo từng slot từ `SaveService.ListSlots()` (S5), state tree dump. Sau Load, copy field vào `GameContext.World` hiện có + publish `WorldStateReloaded` (S5) | ⬜ (chưa test tự động, chỉ verify code compile + wiring scene) |
...
## Data định nghĩa game — CONTENT THẬT + SCENE ĐÃ NỐI (S5+S6+S7)

`Assets/StreamingAssets/Definitions/` — `manifest.json` (`definition_version: 0.3.0`) + `README.md` + content P1 thật:
...
- `balance.json` — khớp default `BalanceConfig` + section `condition` (S7).
- `phases_p2.json` (S7, mới) — 4 `DisasterPhaseDefinition`: dry@0, first_rain@30, black_rain@80, route_closure@140 (game-minute).
...
- **M1 (S2-S4) PASS**, **S5 PASS**, **S6 PASS → Gate P1 PASS về mặt kỹ thuật (2026-07-24)**; **S7 PASS về mặt kỹ thuật (2026-07-24)**: 80/80 EditMode test, build Windows 0 lỗi, headless smoke xác nhận boot + DisasterPhaseSystem/ConditionSystem khởi tạo không exception.
- `DebugOverlay` (F1, Sprint 1) và `DebugPanel` (F2, Sprint 4) là 2 file khác nhau — F1 luôn hiện (FPS/vị trí), F2 toggle riêng (World Time/Save/Add Item/Condition cheat/Travel cheat/state dump).
...
- `StartSleepCommand` fast-forward clock nhưng KHÔNG kiểm tra event/interrupt/Condition (chờ S12 viết lại theo plan P3/P4) — ngủ hiện tại vẫn tích luỹ hunger/thirst/fatigue bình thường qua ShortTick/LongTick trong lúc FastForward.
- S7 chỉ dựng CHAIN cho `black_water` exposure (`ConditionOps.ApplyExposureStatusChain`) — KHÔNG có nguồn nào populate `Exposures["black_water"]` trong gameplay thật P1/P2 tới lúc này (chỉ test unit + DebugPanel cheat `exposure_black_water`). Nguồn thật (crossing route ngập) là việc của S8 `TravelRules.EvaluateCrossing`.
- `ItemDefinition.EquipSlot`/`Protection` (S7) là field rỗng chưa ai dùng — đặt trước cho S8 EquipItemCommand/ProtectionSpec, tránh migrate schema 2 lần.
- **Chưa playtest thật bằng tay/mắt** — mọi xác nhận ở trên là test tự động + headless smoke (không render hình). User nên tự mở Editor hoặc chạy `Builds/Windows/LastHope.exe` để chơi thử, đặc biệt xem F2 Debug Panel hiển thị Condition đúng khi hunger/thirst tăng dần theo thời gian và khi trú tại shelter thân nhiệt hồi phục.
...
- Hệ thống Hazard/Flood + Equipment (S8) và Shelter recovery + Scenario A-D (S9) **CHƯA VIẾT GÌ** — xem `docs/plans/2026-07-24-p1-p2-completion-plan.md`.

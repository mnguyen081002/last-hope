...
| `Assets/Game/Presentation/Boot/SceneFlowController.cs` (S6) | `SceneFlowController` | Chủ sở hữu DUY NHẤT vòng đời scene gameplay: nghe `TravelCompleted`/`WorldStateReloaded` → load additive scene theo `LocationDefinition.SceneName` của `Player.CurrentLocationId`, unload scene cũ, đặt player ở `PlayerSpawnPoint` nếu không có vị trí save khớp. Có `GameLog` ở mỗi bước (verify qua headless smoke: load `20_MainShelter` thành công) | ⬜ (verify qua smoke test, chưa PlayMode test) |

...
| Z-level tầng | `Presentation/World/{FloorLevel,FloorRenderController,StaircaseZone}.cs`, `Presentation/Player/PlayerFloorState.cs` | `FloorLevel.Floor`, `PlayerFloorState.{CurrentFloor,TransitioningToFloor,ClimbProgress,BeginClimb,SetClimbProgress,CancelClimb,TeleportToFloor,ResetFloor}`, `FloorRenderController` (tự tìm `FloorLevel`/`PlayerFloorState` qua `FindObjectsByType`) | ⬜ | P3, dựng lại 3 lần sau nhiều vòng user review (2026-07-28→29): SetActive nhị phân → 2 trigger đổi tức thời → **1 `StaircaseZone` leo dần** (bản hiện tại). `StaircaseZone` — **ngoại lệ duy nhất** không dùng `IInteractable`: `Collider2D.isTrigger`, `OnTriggerEnter2D` bắt đầu leo (`BeginClimb`), `OnTriggerStay2D` tính `ClimbProgress = InverseLerp(bottomY, topY, playerY)` mỗi frame (đi lùi thì tụt lại), `OnTriggerExit2D` huỷ nếu rời vùng trước khi chạm đỉnh. Root riêng ở gốc scene — KHÔNG nằm trong GroundFloor/UpperFloor vì Collider2D 2 root đó bị tắt khi Dimmed/Hidden, cầu thang phải luôn bắt được player từ cả hai phía. `FloorRenderController` khi đang leo (`TransitioningToFloor != null`) nội suy alpha liên tục giữa 2 tầng theo `ClimbProgress` (không nhị phân Full/Dimmed), đổi Collider2D đúng mốc 0.5; khi không leo thì như cũ (Full/Dimmed alpha 0.35 + sortingOrder -1000/Hidden theo hiệu số tầng). `PlayerFloorState` sống trên Player (DontDestroyOnLoad) nên `SceneFlowController.RepositionPlayer` phải gọi `ResetFloor()` mỗi lần đổi scene; `PlacementModeController` dùng `TeleportToFloor` (đổi tức thời, không leo) khi tự chuyển tầng theo Zone đang chọn — thuần Presentation, không qua Command/WorldState/save (giống scope cut "Save/Load không đổi scene" ở P1). Fix đã áp dụng: `FindObjectsByType<FloorLevel>(FindObjectsInactive.Include, sortMode)` — overload không truyền `FindObjectsInactive` mặc định loại trừ GameObject inactive, khiến tầng chưa từng active không bao giờ được quét thấy để bật lên |

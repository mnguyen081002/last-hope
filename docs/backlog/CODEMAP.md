...
| File | Class | API chính | Test |
| --- | --- | --- | --- |
| `Assets/Game/DebugTools/Panel/DebugPanel.cs` | `DebugPanel` (MonoBehaviour, OnGUI, F2) | Xem World Time, Fast-forward clock, Pause/TimeScale (qua `SimulationDriver`), **Condition cheat** (S7, bypass Command Layer), **Equipment** (S8: xem `EquipmentSlots` hiện có + ô nhập instance id/slot → nút Equip/Unequip **qua EquipItemCommand/UnequipItemCommand thật**, không bypass), Add Item (bypass Command Layer), Travel cheat, Save/Autosave + nút Load theo từng slot, state tree dump. Sau Load, publish `WorldStateReloaded` | ⬜ (chưa test tự động, chỉ verify code compile + wiring scene) |
...
## Data định nghĩa game — CONTENT THẬT + SCENE ĐÃ NỐI (S5+S6+S7+S8)

`Assets/StreamingAssets/Definitions/` — `manifest.json` (`definition_version: 0.4.0`) + `README.md` + content thật:
...
- **M1 (S2-S4) PASS**, **S5 PASS**, **S6 PASS → Gate P1 PASS về mặt kỹ thuật (2026-07-24)**; **S7 PASS**; **S8 PASS về mặt kỹ thuật (2026-07-24)**: 116/116 EditMode test, build Windows 0 lỗi, headless smoke xác nhận boot + HazardSystem khởi tạo không exception.
- `DebugOverlay` (F1) và `DebugPanel` (F2) là 2 file khác nhau — F1 luôn hiện (FPS/vị trí), F2 toggle riêng (World Time/Save/Condition cheat/Equipment/Add Item/Travel cheat/state dump).
- `StartTaskCommand`/`CancelTaskCommand` vẫn chỉ validate + ghi flag/log, KHÔNG có effect thật — đó là việc của Shelter Task (S10+). `BeginTravelCommand` thì ĐÃ đầy đủ từ S6, và từ S8 có validate hazard + chi phí crossing.
...
- **`black_water` exposure giờ có 2 nguồn thật** (S8): băng qua route ngập (`BeginTravelCommand`) và cầm đồ Contaminated không gloves (`TransferItemCommand`). Không còn dormant như ghi chú S7 cũ.
- `items_p2.json` (jacket/boots/gloves/rope/dry_bag/medkit) **chưa gắn vào searchpoint/loot table nào** — chưa có cách người chơi NHẶT được trong gameplay thật, chỉ test qua debug Add Item + EquipmentRulesTests/CommandPipelineTests. Đặt loot thật là việc content của S9 hoặc P3+.
- **Không có `ProtectionSpec.cs` riêng** dù plan gốc S8 có nhắc tên file này — quyết định dùng `ItemDefinition.Protection:Dict<string,float>` (đã có sẵn từ S7) vì đủ biểu diễn mọi hiệu ứng cần (rope/jacket/boots/gloves/dry bag) mà không cần thêm 1 lớp cấu trúc song song.
...
- **Chưa playtest thật bằng tay/mắt** — mọi xác nhận ở trên là test tự động + headless smoke (không render hình). User nên tự mở Editor hoặc chạy `Builds/Windows/LastHope.exe` để chơi thử: mở World Map (phím M) xem route hiện Flood/Current/ETA đúng, và F2 Debug Panel Equipment section equip/unequip qua id thật.

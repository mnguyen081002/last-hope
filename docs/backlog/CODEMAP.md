...
**P3 — Shelter Loop: code xong toàn bộ (S10-S13/BL-P3-01..18), chờ user playtest** (2026-07-28).
219 EditMode test xanh (166 P1/P2 + 53 mới), build Windows + smoke test headless pass. Phạm vi
rút gọn có chủ đích so với full design doc — xem `docs/plans/2026-07-28-p3-shelter-loop.md`
(không dựng multi-floor vật lý/cầu thang, Elevated Storage là modifier không phải kho riêng,
Task System = Construction, mọi tương tác qua 1 prop "Shelter Console"). Số liệu Event/Sleep
(storage_flood_loss_chance, drain_backflow_trigger_chance, pump_jam_chance, sleep_fatigue_
recovery...) là tự đề xuất, chưa qua playtest — ghi `_note_p3` trong `balance.json`. Chưa có:
P4.
...
| Hệ thống | File | API chính | Test | Ghi chú |
| --- | --- | --- | --- | --- |
| Definition types | `Data/Definitions/*.cs` | `ItemDefinition`, `LocationDefinition`, `RouteDefinition`, `SearchPointDefinition`, `ModuleDefinition`, `ShelterZoneDefinition`, `BalanceDefinition` | ✅ | Khớp schema snake_case sẵn có. `ModuleDefinition`/`ShelterZoneDefinition` (P3) khớp `modules_p3.json`/`shelterzones_p3.json` |
...
| Telemetry | `Systems/Telemetry/TelemetryLogger.cs` | `LogSearchClosed`, `LogInventoryOpenDuration` (+ tự subscribe Travel/Location/Search event) | ⬜ | JSONL `persistentDataPath/Telemetry/session_*.jsonl`. Sự kiện có EventBus sẵn thì tự nghe; sự kiện chỉ UI biết (đóng panel, thời gian mở) UI gọi thẳng. **Chưa nối P3** (Shelter event/construction) |
...
| Build | `Systems/Shelter/BuildSystem.cs` | `CanStartConstruction`, `StartConstruction/CancelConstruction/SetPaused/DismantleModule`, `ApplyShortTick` | ✅ | Chỉ 1 construction cùng lúc (MVP). Tick mỗi phút qua `ShortTick` — tự chạy dù rời Shelter/Sleep (Passive Task = hệ quả của kiến trúc tick sẵn có, không cần abstraction riêng) |
...
| Z-level tầng | `Presentation/World/{FloorLevel,FloorRenderController,FloorTransitionTrigger}.cs`, `Presentation/Player/PlayerFloorState.cs` | `FloorLevel.Floor`, `PlayerFloorState.SetFloor/ResetFloor`, `FloorRenderController` (tự tìm `FloorLevel`/`PlayerFloorState` qua `FindObjectsByType`) | ⬜ | P3 (2026-07-28, dựng lại sau khi user yêu cầu review — bản đầu chỉ SetActive nhị phân, giờ là Z-level thật kiểu Project Zomboid). `FloorTransitionTrigger` — **ngoại lệ duy nhất** không dùng `IInteractable`: `Collider2D.isTrigger` + `OnTriggerEnter2D`, đi bộ qua là đổi tầng (không bấm phím, vì đổi tầng là di chuyển thuần tuý không có hệ quả cần xác nhận). `FloorRenderController` áp Full/Dimmed (alpha 0.35 + tắt Collider2D + đẩy sortingOrder xuống -1000)/Hidden (SetActive false) theo hiệu số tầng — tầng dưới hiện mờ (thấy bố cục, không va chạm được) thay vì ẩn hẳn, đúng cảm giác PZ. `PlayerFloorState` sống trên Player (DontDestroyOnLoad) nên `SceneFlowController.RepositionPlayer` phải gọi `ResetFloor()` mỗi lần đổi scene — thuần Presentation, không qua Command/WorldState/save (giống scope cut "Save/Load không đổi scene" ở P1) |
...
| Shelter | `UI/Panels/ShelterPanel.cs` | — | ⬜ | P3, tự mở khi nghe `ShelterConsoleOpened` (toggle). Toàn bộ Zone/Slot trong một panel (phạm vi rút gọn — không đi bộ tới từng Zone vật lý). Overview (Structural/Water Intrusion/Clean-Untreated Water/Battery/Filter), banner 3 Event (Drain Backflow/Storage Flood Risk/Pump Jam) + nút xử lý, mỗi Slot: trống (danh sách Module xây được + nút Xây), đang xây (progress + Tạm dừng/Huỷ), đã xây (Durability/Powered/nút đổi Power Priority xoay vòng/Tháo) |
...
| `10_GamePersistent` | `GameServices` (Bootstrapper+Driver+DebugPanel), `Player` (Controller+AvatarSync+OverloadSync+InteractionDetector), `Main Camera`+`CameraRig`, `DebugOverlay`, `InteractionPrompt`, `SceneFlowController`, `InventoryPanel`, `SearchPanel`, `StoragePanel`, `TravelConfirmPanel`, `ShelterPanel`, `SleepPanel` (P3, 2026-07-28) |

...
| World views | `Presentation/World/{SearchPointView,StorageView,TravelPointView,PlayerSpawnPoint,ShelterConsoleView,BedView,StaircaseView}.cs` | implement `IInteractable` | ⬜ | Chỉ submit Command/publish event, không biết UI nào phản hồi (tránh phụ thuộc `UI`). `PlayerSpawnPoint.RouteId` (2026-07-27) — scene nhiều `TravelPoint` thì nhiều spawn, mỗi cái gắn đúng route dẫn tới nó. `TravelPointView.Interact()` (2026-07-28, BL-P2-11) đổi từ submit thẳng `BeginTravelCommand` sang publish `TravelPointOpened` — `TravelConfirmPanel` (UI) mới thật sự submit sau khi user bấm "Xác nhận". `ShelterConsoleView`/`BedView` (P3) publish `ShelterConsoleOpened`/`BedOpened` — không có payload, chỉ một Shelter. `StaircaseView` (P3, 2026-07-28) — đổi tầng Ground↔Upper trong `20_MainShelter`: `SetActive` 2 GameObject root cùng footprint + `PlayerAvatarSync.TeleportTo`, thuần Presentation không qua Command/WorldState (giống scope cut "Save/Load không đổi scene" ở P1 — load lại luôn về Ground) |
...
| `Shelters/20_MainShelter` | `GroundFloor` (active mặc định: `StorageView`, `ShelterConsoleView`, 2 `TravelPointView` → store/gara, 2 `PlayerSpawnPoint`, `StaircaseView` lên) + `UpperFloor` (inactive mặc định: `BedView`, `StaircaseView` xuống) — 2 root cùng footprint world, toggle bằng `SetActive` (P3, 2026-07-28) |

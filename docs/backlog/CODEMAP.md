...
225 EditMode test xanh, build Windows + smoke test headless pass. Ground/Upper Floor là Z-level
...
| Shelter state | `Core/State/ShelterState.cs` | `ShelterState`, `BuiltModuleState`, `ConstructionState`, `PowerPriority` | ✅ | Chỉ một Main Shelter trong MVP (không key theo LocationId). Vật liệu xây dựng vẫn dùng `LocationState.StorageContainer` sẵn có (P1). Đổi 2026-07-30: `ConstructionState` chỉ còn `ModuleId`/`MinutesRemaining`/`Paused` (bỏ Zone/vị trí/rotation — Production tách khỏi Placement); thêm `ShelterState.ReadyToClaim` (Dictionary ModuleId→count, độc lập khỏi Construction, persist qua serializer sẵn có) |
...
| Events | `Core/Events/EventBus.cs`, `GameEvents.cs` | `Subscribe/Unsubscribe/Publish<T>` | 🟡 | struct event, handler copy-on-write. Có: `WorldTimeChanged`, `InventoryChanged`, `LocationChanged`, `SearchPointOpened`, `StorageOpened`, `TravelPointOpened`, `TravelStarted`, `WorldStateReloaded`, `ShelterConsoleOpened`/`BedOpened`/`ShelterEventTriggered`/`ConstructionStarted`/`ConstructionCompleted`/`ProductionClaimed`/`PowerPriorityChanged`/`BeginPlacementMode`/`ModuleDismantled`/`ModuleRedeployed` (P3). Đổi 2026-07-30 (Production tách khỏi Placement): `ConstructionStarted`/`ConstructionCompleted` chỉ còn `ModuleId` (bỏ `ZoneId`/`PlacementId` — Production không gắn vị trí, hoàn thành không tự đặt vào thế giới); `BeginPlacementMode` chỉ còn `ModuleId` (bỏ `ZoneId`/`Redeploy` — Zone tự resolve ở `PlacementModeController`, chỉ còn 1 luồng Place); `ProductionClaimed` mới (Nhận Ready to Claim) |
...
| Save | `Core/Save/WorldStateSerializer.cs`, `SaveFile.cs`, `SaveService.cs` | `Save/Load/SaveAutosave`, `PathForSlot` | ✅ | SHA256 checksum, atomic tmp→verify→.bak→rename, autosave 3 slot xoay vòng. `SaveFile.CurrentSaveVersion` = 3 (2026-07-30, đổi schema `ConstructionState`/`ShelterState.ReadyToClaim`) — không có migration, `SaveService` fail-fast nếu version lệch (dự án chưa release) |
...
| Commands | `Systems/Commands/{TransferItemCommand,OpenSearchPointCommand,TakeAllFromSearchPointCommand,BeginTravelCommand,EquipItemCommand,UnequipItemCommand,StartConstructionCommand,CancelConstructionCommand,ClaimProductionCommand,RedeployModuleCommand,DismantleModuleCommand,SetPowerPriorityCommand,ResolveDrainBackflowCommand,RepairPumpJamCommand,SleepCommand}.cs` | implement `IGameCommand` | ✅ | `TransferItemCommand` dùng chung cho Take/Store/Withdraw/Drop/PickUp qua `InventoryOwner`. Command P3 (build/power/event/sleep) — `ClaimProductionCommand` mới 2026-07-30 (Nhận Ready to Claim, cộng packed item vào túi Player) |
| Telemetry | `Systems/Telemetry/TelemetryLogger.cs` | `LogSearchClosed`, `LogInventoryOpenDuration` (+ tự subscribe Travel/Location/Search/Construction/Redeploy/PowerPriority/ShelterEvent) | ⬜ | JSONL `persistentDataPath/Telemetry/session_*.jsonl`. Sự kiện có EventBus sẵn thì tự nghe; sự kiện chỉ UI biết (đóng panel, thời gian mở) UI gọi thẳng. P3 (BL-P3-18, 2026-07-29) — nối `ConstructionStarted`/`ConstructionCompleted` (Build Choice + thời gian chờ Task = chênh `world_time_minutes` 2 dòng log), `PowerPriorityChanged` (Power Allocation choice), `ShelterEventTriggered` (3 Shelter Event). `ConstructionStarted`/`PowerPriorityChanged` là event mới, publish từ `StartConstructionCommand`/`SetPowerPriorityCommand`. Thêm 2026-07-29 (đợt Redeploy) — `ModuleRedeployed` log `module_redeployed`, Build Choice khác (tức thì, không có `ConstructionStarted` đi kèm) |
...
| Build | `Systems/Shelter/BuildSystem.cs` | `CanStartProduction/StartConstruction/CancelConstruction/SetPaused`, `CanClaim/ClaimProduction`, `CanRedeployAt/RedeployModule`, `CanDismantle/DismantleModule`, `ApplyShortTick`, `TryFindModuleByPackedItem` | ✅ | **Đổi 2026-07-30 (Production tách khỏi Placement + Ready to Claim)**: `CanPlaceAt`/`StartConstruction(zoneId,x,y,...)` cũ đã xoá. `StartConstruction(moduleId)` không còn vị trí — chỉ check `CanStartProduction` (Module tồn tại + không có Construction khác + đủ Materials). `ApplyShortTick` xong tăng `ShelterState.ReadyToClaim[moduleId]` thay vì tự tạo `BuiltModuleState`. `CanClaim`/`ClaimProduction` (mới) — Nhận cộng `PackedItemId` vào **túi Player** (`InventorySystem.Add`), reason `NothingToClaim`/`InventoryFull`. `CanRedeployAt`/`RedeployModule` (đặt Module đã gói, dùng chung hình học `CanPlaceGeometry` — Zone/Bounds/Overlap, `ModuleDefinition.FootprintWidth`×`Height`) giờ đọc/trừ `PackedItemId` từ **túi Player** (trước đó Storage) — nguồn duy nhất đi qua `PlacementModeController`, Zone không còn tham số cố định từ UI mà do caller (Presentation) tự resolve mỗi frame. `DismantleModule` cộng `PackedItemId` vào túi Player (trước đó Storage), `CanDismantle` chặn nếu túi đầy. `TryFindModuleByPackedItem` (mới) — tra ngược Module từ 1 itemId, dùng ở `InventoryPanel`. Chỉ 1 Production cùng lúc (MVP). Tick mỗi phút qua `ShortTick` — tự chạy dù rời Shelter/Sleep. `ShelterState.PlacedModules` key theo placementId tự sinh (`NextPlacementId` counter) |
...
| Free Placement | `Presentation/World/PlacementModeController.cs` | bật khi nghe `BeginPlacementMode` | ⬜ | P3 (2026-07-28) — tương tác chuột đầu tiên trong game (mọi thứ khác dùng phím + OnGUI). Ghost `SpriteRenderer` (dựng runtime bằng `Sprite.Create(Texture2D.whiteTexture,...)` — an toàn ở build) theo con trỏ, xanh/đỏ theo `BuildSystem.CanRedeployAt`, khung mờ biên Zone. Click trái xác nhận submit `RedeployModuleCommand` (tức thì — luồng Place duy nhất còn lại), ESC (action `Close`) huỷ. Rotate (phím R, `RotateModule` action) 90°/lần nếu `ModuleDefinition.IsRotatable` + đủ 4 sprite hướng (`ModuleSpriteCatalog.HasAllDirections`), re-validate ngay. **Đổi 2026-07-30 (Production tách khỏi Placement)**: `BeginPlacementMode` chỉ mang `ModuleId` — không còn ZoneId cố định. Zone tự resolve **mỗi frame** từ `ModuleDefinition.AllowedZoneIds` lọc theo `PlayerFloorState.CurrentFloor` + `ShelterZoneDefinition.Contains(x,y)` (hàm `ResolveZoneId`); không zone nào khớp → reject `OutOfBounds`. Không còn auto-teleport tầng lúc mở (trước đó biết trước đúng 1 Zone/tầng) — Module có nhiều Zone khả dĩ ở nhiều tầng (`module_elevated_storage`: `ground_storage`/`upper_living`) thì người chơi tự đi cầu thang đổi tầng trong lúc ghost đang mở; khung mờ biên Zone (`zoneBoundsBox`) cũng tạo/huỷ lại theo zone resolve được mỗi frame thay vì cố định lúc vào. Sửa 2026-07-29 (ghost đỏ lệch chuột): field `worldCamera` tường minh thay `Camera.main` |
| Module đã xây (visual) | `Presentation/World/PlacedModuleRenderer.cs` | tự đồng bộ khi nghe `ModuleDismantled`/`ModuleRedeployed` | ⬜ | P3 (2026-07-29) — user báo xây xong Module không thấy hiện trong thế giới. `SyncAll()` mỗi lần nghe event: diff toàn bộ `PlacedModules` hiện tại với Dictionary view đang giữ — không tin incremental theo event đơn lẻ. Sprite theo hướng qua `ModuleSpriteCatalog.Load` (fallback `propSprite` placeholder nếu Module chưa có art). Parent đúng `FloorLevel` (Ground/Upper theo `ShelterZoneDefinition.Floor`) để `FloorRenderController` tự mờ/ẩn theo Z-level. Không có `Collider2D` — chưa chặn di chuyển vật lý qua Module. Đổi 2026-07-30: **bỏ subscribe `ConstructionCompleted`** — Production hoàn thành giờ chỉ chuyển "Ready to Claim" (`ShelterState.ReadyToClaim`), không tự tạo `BuiltModuleState`; chỉ còn `ModuleRedeployed`/`ModuleDismantled` thật sự đổi `PlacedModules` |
| Module hover-menu (Tháo) | `Presentation/World/PlacedModuleHoverMenu.cs` | — | ⬜ | P3 (2026-07-29) — user yêu cầu đổi UX Tháo từ nút trong `ShelterPanel` sang world-space, kiểu Project Zomboid/RimWorld: rê chuột dừng trên sprite Module (đúng tầng player + trong bán kính `ProximityRadius=1.6f` quanh player, giống `InteractionDetector.detectionRadius`) hiện menu nút "Tháo". Paradigm tương tác thứ ba trong game (khác `IInteractable` bám player/phím E, khác click-đặt `PlacementModeController`). Menu neo theo **vị trí world của Module** (WorldToScreenPoint mỗi frame, không neo theo chuột — bug ban đầu: neo theo `Event.current.mousePosition` khiến menu "chạy theo" chuột, không bấm được nút) + logic sticky (`menuScreenRect.Contains`) giữ menu mở khi chuột rời world-hover nhưng còn trong chính menu. Bấm "Tháo" submit `DismantleModuleCommand` — packed item về túi Player (đổi 2026-07-30, trước đó Storage). Sửa 2026-07-30: `PointerOverUI.MarkHover(true)` trước gọi vô điều kiện mỗi khi hover Module (không check mouse có thật sự trong rect menu) — đổi thành `MarkHover(rect.Contains(...))` cho nhất quán với mọi panel khác |
...
| Hệ thống | File | API chính | Test | Ghi chú |
| --- | --- | --- | --- | --- |
| Inventory | `UI/Panels/InventoryPanel.cs` | toggle qua action `ToggleInventory` | ⬜ | OnGUI (không phải uGUI — quyết định P1-C, xem plan doc). Hiện túi + Carried Object + đồ dưới đất tại location + khu "Đang mặc" (nút Tháo, hiện thông báo nếu bị từ chối) và nút "Mặc" cạnh item equipment trong túi + thanh progress tải trọng (đầy tới hard cap, 2026-07-27). Đóng: nhấn lại `ToggleInventory` hoặc ESC (`Close`). Thêm 2026-07-30 (Production tách khỏi Placement) — nút "Đặt" cạnh packed item trong túi Player (`BuildSystem.TryFindModuleByPackedItem` tra Module từ itemId), publish `BeginPlacementMode(moduleId)` + đóng panel; `DrawRow` đổi từ static sang instance method để gọi được `SetVisible(false)` |
...
| Shelter | `UI/Panels/ShelterPanel.cs` | — | ⬜ | P3, tự mở khi nghe `ShelterConsoleOpened` (toggle). Toàn bộ Zone trong một panel (phạm vi rút gọn — không đi bộ tới từng Zone vật lý). Overview (Structural/Water Intrusion/Clean-Untreated Water/Battery/Filter), banner 3 Event + nút xử lý. **Đổi 2026-07-30 (Production tách khỏi Placement)**: khu "Sản xuất" đầu panel liệt kê **toàn bộ Module** (không nhóm theo Zone — Production không gắn Zone), nút "Sản xuất" submit `StartConstructionCommand(moduleId)` (disable nếu đang có Construction khác/thiếu vật liệu, `BuildSystem.HasEnoughMaterials`). Construction hiển thị 1 chỗ (chỉ 1 cái chạy cùng lúc), label bỏ Zone (không còn gắn). Khu "Ready to Claim" mới — mỗi `(moduleId, count)` trong `ShelterState.ReadyToClaim` có nút "Nhận" submit `ClaimProductionCommand`. `DrawZone` giờ **chỉ** liệt kê `PlacedModules` đã xây trong Zone đó (`(x,y)` thật, nút đổi Power Priority xoay vòng) — đã bỏ hẳn nút "Chọn vị trí"/"Đặt lại (×N)" (Placement chuyển sang `InventoryPanel`, xem dòng Inventory bên dưới). Nút "Tháo" đã chuyển sang world hover-menu từ 2026-07-29 (`PlacedModuleHoverMenu`), không đổi lại lần này |
